...
def pridict():
    ...
    if response.status_code == 200:
        ...
        Label(text = temperature,fg="blue").grid(row = 3,column=2)
        
        Label(text = "humidity : ").grid(row = 4,column=1)
        Label(text = humidity,fg="blue").grid(row = 4,column=2)
        
        Label(text = "description : ").grid(row = 5,column=1)
        Label(text = description,fg="blue").grid(row = 5,column=2)
        
        Label(text = "wind_speed : ").grid(row = 6,column=1)
        Label(text = wind_speed,fg="blue").grid(row = 6,column=2)
...
Label(text = "Weather Forcast", fg = "yellow", font=("Times",20,"bold",UNDERLINE)
      ).grid(row=1,column=2)

Label(text = "Enter City / city Code : ",padx=5,pady=10,font=("times",13)).grid(row=2,column=1)

valueinput = StringVar()
valueEntry = Entry(root, textvariable=valueinput)
valueEntry.grid(row=2, column=2)

Button(text = "Predict",command=pridict,bg="blue",fg="white",width=5).grid(row = 2,column=3)
